    ...
    while (gap > 1 || swapped) {
        ...
        for (int i=0; i+gap<size; i++) {
            if ((int)arr[i]>(int)arr[i+gap]) {
                ...
                swapped = true;
            } else if ((int)arr[i]<=(int)arr[i+gap]) {
                continue;
            }
        }
    }
...
void writeAnalysis(const char* filename, double durationSec, int size) {
    ...
    analysisFile << "Comb Sort running time: " << durationSec << " sec (size = " << size << ")\n";
    analysisFile.close();
}
...
int main(int argc, char* argv[]) {
    ...
    if (input_type == "sorted_inc") {
        ...
        output_file = "sorted_nums/comb_sorted_inc.txt";
        analysis_file = "analysis/comb_sorted_inc.txt";
    } else if (input_type == "sorted_dec") {
        input_file = "input_data/sorted_dec.txt";
        output_file = "sorted_nums/comb_sorted_dec.txt";
        analysis_file = "analysis/comb_sorted_dec.txt";
    } else if (input_type == "random") {
        input_file = "input_data/random.txt";
        output_file = "sorted_nums/comb_random.txt";
        analysis_file = "analysis/comb_random.txt";
    } else if (input_type == "partial") {
        input_file = "input_data/partial.txt";
        output_file = "sorted_nums/comb_partial.txt";
        analysis_file = "analysis/comb_partial.txt";
    } else if (input_type == "test") {
        input_file = "input_data/test.txt";
        output_file = "sorted_nums/comb_test.txt";
        analysis_file = "analysis/comb_test.txt";
    } else if (input_type == "stability") {
        input_file = "input_data/stability.txt";
        output_file = "sorted_nums/comb_stability.txt";
        analysis_file = "analysis/comb_stability.txt";
    }
